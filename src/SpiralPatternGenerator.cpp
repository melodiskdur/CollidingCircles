...
void SpiralPatternGenerator::generateMainArm(const GLfloat& initialRotationDeg, const GLfloat& degStep)
{
    for (GLsizei i{ 0 }; i < m_params->m_numCirclesPerMainArm; i++)
    {
        ...
        std::uniform_real_distribution<float> coordFuzz(-m_params->m_fuzzFactor * m_params->m_discRadius,
                                                        m_params->m_fuzzFactor * m_params->m_discRadius);
        glm::vec2 position{ m_center.x + expFactor * glm::cos(glm::radians(initialRotationDeg) + theta) + 0.03f * coordFuzz(m_rgen),
                            m_center.y + expFactor * glm::sin(glm::radians(initialRotationDeg) + theta) + 0.03f * coordFuzz(m_rgen) };
        glm::vec2 velocity{ 1.0f / length(position - m_center) * glm::vec2((position - m_center).y, -(position - m_center).x) };
...
void SpiralPatternGenerator::generateSubArm(const GLfloat& initialRotationDeg, const GLfloat& degStep)
{
    for (GLsizei i{ 0 }; i < m_params->m_numCirclesPerSubArm; i++)
    {
        ...
        std::uniform_real_distribution<float> coordFuzz(-m_params->m_fuzzFactor * m_params->m_discRadius,
                                                        m_params->m_fuzzFactor * m_params->m_discRadius);
        glm::vec2 position{ m_center.x + expFactor * glm::cos(glm::radians(initialRotationDeg) + theta) + 0.08f * coordFuzz(m_rgen),
                            m_center.y + expFactor * glm::sin(glm::radians(initialRotationDeg) + theta) + 0.08f * coordFuzz(m_rgen) };
